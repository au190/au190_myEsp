...
class ArduinoBootloader(object):
    ...
    async def select_programmer(self, protocol, websocket):
    ...
    async def _is_cpu_signature(self, signature):
    ...
    class Stk500v1(object):
        ...
        async def get_sync(self):
            ...
            for i in range(1, 10):
                #time.sleep(0.05)
                if await self._cmd_request(b'\x30\x20', answer_len=2):
                    #time.sleep(0.05)
                    return True
            return False

        async def board_request(self):
            """Get the firmware and hardware version of the bootloader.

            :return: True when success.
            :rtype: bool
            """
            if await self._cmd_request(b"A\x80 ", answer_len=3):#
                self._ab._hw_version = self._answer[1]

            if await self._cmd_request(b"A\x81 ", answer_len=3):#STK_SW_MAJOR
                self._ab._sw_major = self._answer[1]

            if await self._cmd_request(b"A\x82 ", answer_len=3):#STK_SW_MINOR
            ...
            response is used."""
            if not await self._cmd_request_no_len(b"1 ", answer_len=7):
                return False
        ...
        async def cpu_signature(self):
            ...
            if await self._cmd_request(b"u ", answer_len=5):
                return await self._ab._is_cpu_signature((self._answer[1] << 16) | (self._answer[2] << 8) | self._answer[3])
            return False

        async def write_memory(self, buffer, address, flash=True):
            ...
            :rtype: bool
            """
            if await self._set_address(address, flash):
                buff_len = len(buffer)
                ...
                return await self._cmd_request(cmd, answer_len=2)
            return False

        async def read_memory(self, address, count, flash=True):
            ...
            if await self._set_address(address, flash):
                ...
                if await self._cmd_request(cmd, answer_len=count+2, clear_buff=False):
                    # The answer start with RESP_STK_IN_SYNC and finish with RESP_STK_OK
                    print((datetime.utcnow().strftime('%H:%M:%S.%f')[:-3]) + " <-- " + str(len(self._answer)) )
                    buffer = bytearray(self._answer[1:count+1])
                    return buffer
                else:
                    print((datetime.utcnow().strftime('%H:%M:%S.%f')[:-3]) + " <-- Nok " + str(len(self._answer)) )
            return None

        async def _set_address(self, address, flash):
        ...
        async def leave_bootloader(self):
            """Leave programming mode and start executing the stored firmware

            :return: True when success.
            :rtype: bool
            """
            return await self._cmd_request(b"Q ", answer_len=2)

        async def _cmd_request_no_len(self, msg, answer_len, clear_buff = True):
            ...
            #print((datetime.utcnow().strftime('%H:%M:%S.%f')[:-3]) + " --> send: " + str(msg))
            await self.websocket.send(msg)

            self._answer.clear()
            start_time = await self.millis() + 1000

            while (await self.millis() < start_time):
                try:

                    data = await self.websocket.recv()
                    #print((datetime.utcnow().strftime('%H:%M:%S.%f')[:-3]) + " <-- " + str(data))

                    if ord(data) == RESP_STK_OK:  # end of the response

                        self._answer.append(ord(data))

                        """If the answer has at least two characters, check that the first and last corresponds to the start and end sentinel."""
                        if len(self._answer) >= 2 and self._answer[0] == RESP_STK_IN_SYNC and self._answer[-1] == RESP_STK_OK:

                            #print((datetime.utcnow().strftime('%H:%M:%S.%f')[:-3]) + " <-- rcv Ok: " + str(len(self._answer)))
                            return True

                        else:  # For read I need all the info
                            if clear_buff:
                                #print((datetime.utcnow().strftime('%H:%M:%S.%f')[:-3]) + " ### Error read buffer clearing .... ")
                                self._answer.clear()
        ...
        async def _cmd_request(self, msg, answer_len, clear_buff = True):
            ...
            :rtype: bool
            """
            #print("clear_buff: " + str(clear_buff))
            if await self._cmd_request_no_len(msg, answer_len, clear_buff):
                return len(self._answer) == answer_len
            return False

        async def millis(self):
    ...
    class Stk500v2(object):
        ...
        async  def _send_command(self, cmd, data=None):
            ...
            await self.websocket.send(buff)
            return True


        async def _recv_answer(self, cmd):
            ...
            if not head is None:
                ...
                if len_data >= 3:
                    self._answer = bytearray(await self.websocket.recv())
        ...
        async def _read_headear(self):
            ...
            for i in range(1, 10):
                ...
                if len(start) >= 1 and start[0] == MESSAGE_START:
                    head = bytearray(await self.websocket.recv())
